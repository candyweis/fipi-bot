...
async def check_url_changes(context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    try:
        ...
        # ИСПРАВЛЕНО: используем asyncio.to_thread вместо context.application.loop
        try:
            # Для Python 3.9+
        ...
        if previous_count is None:
            # Первая проверка - сохраняем количество без уведомлений
        ...
        if previous_count != current_count:
            ...
            # Проверяем, не запущен ли уже парсинг для этого URL
            if url in active_auto_parsing:
                log.info(f"⏳ Парсинг для {subject_name} уже активен, пропускаю")
                return

            # Добавляем в активные парсинги
            active_auto_parsing.add(url)

            try:
                # Уведомляем пользователей об изменении количества
...
async def start_automatic_parsing(context: ContextTypes.DEFAULT_TYPE, url: str, current_count: int):
    ...
    # Создаем фейковый query для автоматического парсинга
    class AutoQuery:
        def __init__(self):
    ...
    try:
        # Добавляем в очередь как автоматическую задачу
...
async def notify_id_changes(context: ContextTypes.DEFAULT_TYPE, url: str,
    ...
    for chat_id in subscribers:
        try:
            ...
            # Создаем и отправляем файл с изменениями
...
async def cleanup_old_data():
    ...
    try:
        ...
        # Очищаем историю парсинга (старше 30 дней)
        cleaned_count = 0
        for url in list(store.get("historical_ids", {}).keys()):
            ...
            for record in history:
                try:
                    ...
                    # Оставляем записи с некорректными датами
                    filtered_history.append(record)
            ...
            if len(filtered_history) != len(history):
                ...
                log.info(f"🧹 Очищена история для {subj_by_url(url)}: {len(history)} → {len(filtered_history)}")

        # Очищаем кэш недавних результатов (старше 6 часов)
        from database import clean_old_parsing_cache
        clean_old_parsing_cache()

        # Очищаем пустые записи подписок
        empty_subscriptions = []
...
async def daily_cleanup(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Дополнительная очистка для ежедневного запуска
